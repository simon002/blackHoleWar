...
-- @author zrong
-- Creation: 2014-11-05
-- Modification: 2015-12-04 Modify for Lerna
------------------------------------------

local RM = {}

local ac = cc.AnimationCache:sharedAnimationCache()
local sfc = cc.SpriteFrameCache:sharedSpriteFrameCache()
local tc = cc.TextureCache:sharedTextureCache()
local FU = import('.FileUtil')
-- 2015-12-10 zrong 详见 lerna/init.lua
...
local function _getCCSFile(name)
    if string.find(name, RM.D_CCS) == 1 then
        return name
    elseif string.find(name, '.json') ~= #name - 4 then
        return string.format('%s%s.json', RM.D_CCS, name)
    end
    return string.format('%s%s', RM.D_CCS, name)
end
...
function RM.normalizeFilePath(typ, name)
    ...
    elseif typ == RM.T_CCS then
        normalized = _getCCSFile(name)
    end
    return normalized
end
    ...
    local fillAnimationCache = function()
        for __, ani in pairs(def.animations) do
            ...
            -- 有些则是使用 range
                ...
                local step = 1
                local last = frameStart + frameNum - 1

                spriteFrameNames = {}
                for index = frameStart, last, step do
                    local frameName = string.format(ani.frame_name, index)
                    spriteFrameNames[#spriteFrameNames+1] = frameName
                end
            end
            -- 2015-12-21 zrong 2.x 的绑定不支持使用 table 代替 CCArray
            -- local animation = cc.Animation:createWithSpriteFrames(spriteFrames, 
            --    ani.delay_per_unit, ani.loops)
            -- 使用一个临时的 CCArray 
            local frameArray = cc.Array:create()
            for __, frame in pairs(spriteFrameNames) do
                frameArray:addObject(cc.String:create(frame))
            end
            local animation = cc.Animation:createWithSpriteFrames(frameArray, ani.delay_per_unit)
...
function RM._fillAniSFPath(defFile)
    local def = assert(loadstring(FU.readFile(defFile)))()
...
function RM.getAni(name)
    ...
    return ac:animationByName(name)
end
...
-- 一个 Cocos Studio UI 的资源载入缓存
-- 若提供 asyncHandler，则为异步载入。否则是同步载入。
function RM.addCCS(name, asyncHandler)
    local ccsFile = FU.getFullPath(RM.normalizeFilePath(RM.T_CCS, name))
    local ccs = _ccs[ccsFile]
    if ccs then
        log:warning('ResourceManager.addCCS %s(%s) 已经在缓存中了。', name, ccsFile)
        asyncHandler(name, ccsFile, ccs)
        return true 
    end
    log:info("ResourceManager.addCCS ccs name:%s, defFile:%s, def:%s", 
        name, ccsFile, ccs or 'nil')
    local ccsStr = FU.readFile(ccsFile)
    ccs = json.decode(ccsStr)
    _ccs[ccsFile] = ccs.texturesPng
    local texNum = #ccs.texturesPng
    local curTex = 0
    ...
    -- 调用 addTex 载入动画定义文件中的 png 到缓存中
    for __, tp in pairs(ccs.texturesPng) do
        -- ccs 中的 png 路径已经经过 normalize ，不应该再处理
        RM.addTex(tp, localHandler)
    end
    return false
end

function RM.removeCCS(name)
    local ccsFile = FU.getFullPath(RM.normalizeFilePath(RM.T_CCS, name))
    local ccs = _ccs[ccsFile]
    if not ccs then 
        log:warning('ResourceManager.removeCCS %s(%s) 不在缓存中，不能移除。', name, ccsFile)
        return false 
    end

    _ccs[ccsFile] = nil
    for __, tp in pairs(ccs) do
        RM.removeTex(tp)
    end
    return true
end

function RM.addCCSList(list, asyncHandler)
    local localHandler = nil
    if asyncHandler then
        local amount = #list
        if amount == 0 then
        ...
        localHandler = function(plist, texture)
            amount = amount - 1
            log:info("ResourceManager.addCCSList.localHandler", plist, texture, amount)
            asyncHandler(amount)
        end
    end
    for __, ss in pairs(list) do
        RM.addCCS(ss, localHandler)
    end
end

function RM.removeCCSList(list)
    for __, name in pairs(list) do
        RM.removeCCS(name)
    end
end
    ...
    if amount > 0 then
       if amount%2 ~= 0 then
           ...
           for i=1, amount, 2 do
               ...
               elseif list[i] == RM.T_CCS then
                   amounts[RM.T_CCS] = #list[i+1]
               else
...
function RM.addResourceList(asyncHandler, ...)
    ...
    if checkSucc then
        ...
        for i=1, #list, 2 do
            ...
            elseif list[i] == RM.T_CCS then
                RM.addCCSList(list[i+1], function(amount)
                    updateAmount(amount, RM.T_CCS)
                end)
            end
        end
    end
...
function RM.removeResourceList(...)
    ...
    if RM._checkResourceList(list) then
        ...
        for i=1, amount, 2 do
            ...
            elseif list[i] == RM.T_CCS then
                RM.removeCCSList(list[i+1])
            end
        end
    end
end

-- 2015-12-21 zrong 在 2.x 废弃 
--[[
