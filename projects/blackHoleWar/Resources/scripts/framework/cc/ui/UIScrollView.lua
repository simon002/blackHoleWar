...
function UIScrollView:scrollToEnd()
    local cascadeBound = self:getScrollNodeRect()
    local viewRect = self:getViewRectInWorldSpace()
    if self:isSideShow() then
        if self.direction == UIScrollView.DIRECTION_VERTICAL then
            local height =  cascadeBound.height - viewRect.height
            local posx = self.scrollNode:getPositionX()
            local posy = self.scrollNode:getPositionY()
            self:scrollTo(posx, posy+height)
        else
            local width =  cascadeBound.width - viewRect.width
            local posx = self.scrollNode:getPositionX()
            local posy = self.scrollNode:getPositionY()
            self:scrollTo(posx+width, posy)
        end
    end
end
