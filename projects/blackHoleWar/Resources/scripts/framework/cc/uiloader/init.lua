...
-- 从json文件克隆一个ui节点，可用于listview的item创建
function uiloader:cloneUINode( jsonFile, nodeName )
    local json = self:loadFile_(jsonFile)
    local function seekNodeUINode( node, targetName )
        if node.options and node.options.name == targetName then
            return node
        else
            for k,v in pairs(node.children) do
                local findNode = seekNodeUINode(v, nodeName)
                if findNode then
                    return findNode
                end
            end
    ...
    local nodeUI = seekNodeUINode(json.widgetTree, nodeName)
    if nodeUI then
        return CCSUILoader:generateUINode(nodeUI)
    end
end
